...
Calculates a dimensionless QCM bridging quantum mechanics, consciousness metrics,
and gravity. Includes sensitivity analysis and visualization of QCM vs. MACRO_SCALE
and CBM using Matplotlib.
...
# Try importing Matplotlib for plotting, but allow code to run without it
try:
...
def plot_sensitivity(results: List[Tuple[float, float, float]]) -> None:
    """Plot QCM vs. MACRO_SCALE for different CBM values."""
    if not PLOTTING_AVAILABLE:
        print("Cannot generate plot: Matplotlib is not installed.")
        return

    # Organize data by CBM
    cbms = sorted(set(cbm for _, cbm, _ in results))
    macro_scales = sorted(set(macro_scale for macro_scale, _, _ in results))
    qcm_data = {cbm: [] for cbm in cbms}

    for macro_scale, cbm, qcm in results:
        if cbm in qcm_data and macro_scale in macro_scales:
            qcm_data[cbm].append(qcm)

    # Create log-log plot
    plt.figure(figsize=(10, 6))
    for cbm in cbms:
        plt.plot(macro_scales, qcm_data[cbm], marker='o', label=f'CBM = {cbm:.1e} J')

    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel('Macro Scale (m)')
    plt.ylabel('QCM (Dimensionless)')
    plt.title('QCM Sensitivity Analysis: QCM vs. Macro Scale for Different CBM')
    plt.grid(True, which="both", ls="--")
    plt.legend()
    plt.tight_layout()

    # Save and display plot
    plt.savefig('qcm_sensitivity.png')
    plt.show()
    logging.info("Sensitivity plot saved as 'qcm_sensitivity.png'.")
...
def derive_scaling_factor() -> float:
    """Attempt to derive the scaling factor theoretically."""
    scale_ratio = (MACRO_SCALE / PLANCK_LENGTH) ** 2
    grav_factor = (C**2) / (GRAVITY * MACRO_SCALE)
    estimated_scale = scale_ratio * grav_factor
    logging.info(f"Derived scaling factor: scale_ratio={scale_ratio:.3e}, "
                 f"grav_factor={grav_factor:.3e}, estimated={estimated_scale:.3e}")
    return estimated_scale
